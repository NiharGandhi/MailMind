...
export async function POST(req: NextRequest) {
  ...
  const { result, promptTokens, completionTokens } = await provider.generateReply(ctx)

  await logSuggestion({
    userId, threadId, suggestionType: 'reply',
    modelUsed: provider.modelId, promptTokens, completionTokens, content: result,
  })
